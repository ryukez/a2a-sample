...
function printAgentEvent(
  ...
  if ("status" in event) {
    const update = event as schema.TaskStatusUpdateEvent; // Cast for type safety
  ...
  // Check if it's a TaskArtifactUpdateEvent
    ...
    const update = event as schema.TaskArtifactUpdateEvent; // Cast for type safety
    console.log(
...
async function fetchAndDisplayAgentCard() {
  ...
  try {
    ...
    if (response.ok) {
      const card: schema.AgentCard =
        (await response.json()) as schema.AgentCard;
